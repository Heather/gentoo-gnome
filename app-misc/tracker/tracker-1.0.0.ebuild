...
SLOT="0/1.0"
IUSE="cue eds elibc_glibc exif firefox-bookmarks flac gif gsf gstreamer gtk iptc +iso +jpeg laptop libav +miner-fs mp3 nautilus networkmanager pdf playlist rss test thunderbird +tiff upnp-av +vorbis +xml xmp xps" # strigi mediaart
...
RDEPEND="
	...
	!gstreamer? ( !libav? ( || ( media-video/totem media-video/mplayer ) ) )
	>=media-libs/libmediaart-0.1.0
"
#	strigi? ( >=app-misc/strigi-0.7 )
#   mediaart? ( >=media-libs/libmediaart-0.1.0 )
DEPEND="${RDEPEND}
	${PYTHON_DEPS}
	$(vala_depend)
...
src_configure() {
	...
	# FIXME: force-enabling mediaart - see upstream bug #728494
	...
	#   $(use_enable libmediaart libmediaart)
}
